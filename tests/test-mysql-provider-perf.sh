...
KEEP_DB="${KEEP_DB:-0}"
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-wpkdb-mysql-perf}"
...
echo "[wp-knowledge] running mysql perf tests with ${WP_KDB_TEST_MYSQL_URL} (compose project=${COMPOSE_PROJECT_NAME} perf rows=${WP_KDB_PERF_ROWS} ops=${WP_KDB_PERF_OPS} hotset=${WP_KDB_PERF_HOTSET})"
